...
async function loadPublicProofs(container) {
    ...
    const withdrawSnap = await getDocs(collection(db, "withdraw_requests"));
    ...
    withdrawSnap.forEach((docSnap) => {
        ...
        if (data.number) {
            ...
            if (num.length >= 11) {
                maskedNumber = num.substring(0, 5) + "****" + num.substring(num.length - 2);
            } else {
                maskedNumber = "Hidden";
            }
        }
    ...
    if (!found) {
        container.innerHTML += "<p>No Payment Proof Available Yet</p>";
    }
